	...
	<table v-if="productArr.length>0" class="pure-table pure-table-horizontal">
		...
		<tbody>
			<tr v-for="(product, index) in productArr" :key="index">
				...
				<td>{{product.price}}</td>
				<td>
					<button @click="productArr.splice(index, 1);removeFromCart(product.id)" 
...
<style scoped>
	...
	.pure-img {width:60px;height:60px;}
	.button-del {color:#fff;background:rgb(202, 60, 60)}
	.count-row td {padding:15px 10px;font-size:16px;font-weight:bold;}
</style>